...
public:
		...
		for (int i = 0; i < numRows; i++){
			for (int j = 0; j < numCols; j++){
				...
				if (memory_->minfo.maze[j][i] == EXIT_CHAR){
					std::cout << "coordinates: " << j << "," << i << "," << memory_->minfo.maze[j][i] << std::endl;
					exit_[COL_IDX] = j;
					exit_[ROW_IDX] = i;
				}
			}
		}
		std::cout << "exit coordinates: " << exit_[COL_IDX] << "," << exit_[ROW_IDX] << std::endl;
	}
	...
	*/
	void draw_runners() {
		...
		RunnerInfo& rinfo = memory_->rinfo;
		std::lock_guard<cpen333::process::mutex>mutexUI(mutex_);
